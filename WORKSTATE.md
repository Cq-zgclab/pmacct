...
4. **传输协议** → ⚠️ **可选增强**:
   - 当前: UDP/IPv4 ✓ (RFC 7011 MUST)
   - 可选: TCP支持 (~1-2小时, RFC 7011 MUST)
   - 可选: SCTP支持 (~2-3小时, RFC 7011 OPTIONAL)
   - 可选: IPv6传输 (~30分钟, socket层升级)
...
1. **短期** (1-2天):
   - [ ] JSON输出集成 (IPC传递, ~4小时)
   - [ ] 性能测试 (1000+ pps, ~2小时)
   - [ ] TCP传输支持 (~1-2小时, RFC 7011 MUST)
   - [ ] SCTP传输支持 (~2-3小时, RFC 7011 OPTIONAL)
   
2. **中期** (1-2周):
   - [ ] 规则缓存优化 (避免重复解析)
   - [ ] 批量处理优化
   - [ ] IPv6传输支持 (发送器socket升级)
   - [ ] 错误处理增强
   - [ ] Web可视化界面
   
3. **长期** (1-3个月):
   - [ ] 正式IANA IE编号申请 (通过IETF流程)
   - [ ] 上游pmacct PR提交 (集成到官方版本)
   - [ ] IETF Implementation Report (帮助标准化)
   - [ ] 生产环境部署方案
   - [ ] 多collector互操作性测试
